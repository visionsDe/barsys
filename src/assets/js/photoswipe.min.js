...
class S {
  ...
  M(t) {
    ...
    if (this.pswp.options.allowPanToNext && "x" === s && "x" === t && !e) {
      ...
      ("y" === t && (o.isShifted() || c.min.y === c.max.y)) || this._(t, l);
    return !1;
  }
  S(t) {
    return (
    ...
    const u = () => {
      this.Mt &&
        ...
        Math.abs(p) < 1 && Math.abs(c.velocity) < 50
          ...
          : ((l = Date.now()), e(p + s), (this.Mt = requestAnimationFrame(u))));
    };
...
class R {
  constructor(i, s) {
    ...
    let n = "";
    s.isButton
      ? ((n += "pswp__button "), (n += s.class || `pswp__button--${s.name}`))
      : (n += s.class || `pswp__${s.name}`);
    ...
    if (s.isButton) {
      ...
      let { title: t } = s;
      const { ariaLabel: e } = s;
      "string" == typeof i.options[h + "Title"] && (t = i.options[h + "Title"]),
        t && (r.title = t);
      const n = e || t;
...
const U = {
  ...
  q = {
    ...
    html: {
      isCustomSVG: !0,
      inner:
        '<path fillRule="evenodd" clipRule="evenodd" d="M21.2 16a5.2 5.2 0 1 1-5.2-5.2V8a8 8 0 1 0 8 8h-2.8Z" id="pswp__icn-loading"/>',
...
class st extends class extends class {
  ...
  init() {
    ...
    return (
      ...
      this.element && (this.element.class += " " + t),
